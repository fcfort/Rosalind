...
# and that the letter is found in our destination string. This means that each probability needs to be multiplied by itself to represent the odds of drawing that string essentially twice. So we square each term of our individua l letter probabilities. We end with:
#
...
#
# Now let us extend this to m, n > 1. Each additional letter in our substring (m > 1) represents another letter that must appear twice, once in our substring and another in our destination string.
#
...
#
# So to get our probabilities for this set, we multiply each probability in # our first formula by itself. Which is the same as squaring its:
#
...
# that a destination string of length 5 (n = 5) and a substring of length 3 ( m = 3 ) can match 3 times, from position 0 to 2, 1 to 3, 2 to 4, and 3 to 5. The general formula is:
#
# n - m + 1 (formula 4)
#
# So we have to multiply our expected occurences formula (formula 3) by the number of times the string can fit (formula 4). We end up with a final formula of:
#
