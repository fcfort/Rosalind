...
# print Dumper(\@closeness_functions);

my @min_reversals_array;
...
while(defined(my $line1 = Common::read_line()) and defined(my $line2 = Common::read_line())) {
    ...
    push(@min_reversals_array, $min_reversals);    
    # skip blank line
    <>;
}

print join(" ", @min_reversals_array), "\n";
...
sub rev_search_helper {
    my ($current, $target, $cur_depth, $min_depth, $is_closer_handle) = @_;
...
sub is_closer_tf {
    ...
    return ( 
            $tf->distance($proposal) < $tf->distance($current)
    );
}
